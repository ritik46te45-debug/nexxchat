...
export default function Sidebar({ onOpenProfile }) {
  const { sidebarView, setSidebarView, isMobile, showChatOnMobile } = useUIStore();
  ...
  // ─── Mobile: Fixed bottom tab bar (hidden when viewing an active chat on mobile) ───
  if (isMobile) {
  ...
  // ─── Desktop / Laptop: Vertical sidebar ───
  return (
    <div className="w-[68px] lg:w-[72px] h-full bg-dark-card border-r border-dark-border flex flex-col items-center py-4 flex-shrink-0 z-20">
      {/* User avatar */}
      <button
      ...
      >
        {user?.avatar?.url ? (
          <img
            ...
            className="w-10 h-10 lg:w-11 lg:h-11 rounded-full object-cover ring-2 ring-dark-border group-hover:ring-primary-500/50 transition-all"
          />
        ) : (
          <div className="w-10 h-10 lg:w-11 lg:h-11 rounded-full gradient-primary flex items-center justify-center text-white font-semibold text-sm ring-2 ring-dark-border group-hover:ring-primary-500/50 transition-all">
      ...
      <nav className="flex-1 flex flex-col items-center gap-2 w-full px-2">
        {navItems.map(({ id, icon: Icon, label, badge }) => (
          <button
            ...
            title={label}
            className={`
              relative w-10 h-10 lg:w-11 lg:h-11 rounded-xl flex items-center justify-center transition-all duration-200
              ${sidebarView === id
      ...
      <button
        ...
        title="Logout"
        className="w-10 h-10 lg:w-11 lg:h-11 rounded-xl flex items-center justify-center text-surface-500 hover:text-red-400 hover:bg-red-500/10 transition-all duration-200"
      >
