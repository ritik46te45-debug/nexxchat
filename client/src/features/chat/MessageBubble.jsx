...
import { Check, CheckCheck, Clock, AlertCircle, Reply, Forward, Star, Trash2, Copy, Edit3, MoreHorizontal, Download, Play, Pause, FileText, MapPin, Users2, BarChart3, Smile, Timer } from 'lucide-react';
    ...
    >
      <div className={`max-w-[75%] sm:max-w-[65%] relative`}>
        ...
        <div className={`px-4 py-2 ${isOwn ? 'msg-sent' : 'msg-received'} ${message.replyTo ? 'rounded-t-none' : ''}`}>
          ...
          {/* Attachments (Standard vs View-Once) */}
          {message.isViewOnce ? (
          ...
          {/* Meta: time + status + edited + disappearing */}
          <div className="flex items-center justify-end gap-1.5 mt-1 -mb-0.5">
            {message.expiresAt && (
              <Clock className="w-3 h-3 text-surface-500" title="Disappearing message" />
            )}
